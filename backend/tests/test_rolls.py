...
async def test_create_roll_without_roll_id_auto_generates(auth_client):
    """roll_id is now optional — backend auto-generates one."""
    resp = await auth_client.post("/api/rolls", json={"film_stock": "Kodak"})
    assert resp.status_code == 201
    data = resp.json()["data"]
    assert data["roll_id"] is not None
    assert len(data["roll_id"]) > 0
